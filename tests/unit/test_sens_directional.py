...
def test_directional_derivative():
    ...
    ctx = build_evaluation_context(
        domain=domain,
        source_spec=SourceSpec(mode=SourceMode.THEVENIN, thevenin_vrms=1.0, z_source_real_ohm=50.0, z_ref_ohm=50.0),
        eom_model=DummyEOM(),
        component_limits=ContinuousLimits(l_min_h=1e-9, l_max_h=1e-6, c_min_f=1e-12, c_max_f=1e-9, i_max_a=1.0, v_max_v=100.0),
        ...
        base_grid_points=5,
        objective_config=ObjectiveConfig(z_ref_ohm=50.0, w_gamma=1.0, w_voltage=0.0, w_loss=0.0, w_complexity=0.0, voltage_targets_rms_v=(), voltage_target_weights=()),
    ...
    _, J = txn.evaluate_jacobians(x0)
    
    d = np.array([0.1, -0.2, 0.05])
    eps = 1e-6
    
    from foster_eom.optimize.evaluator import evaluate, DomainEvaluatorCache
    cache = DomainEvaluatorCache()
    res_plus = evaluate(x0 + d * eps, ctx, cache)
    res_minus = evaluate(x0 - d * eps, ctx, cache)
    
    g_plus = np.array(res_plus.hard_margins)
    g_minus = np.array(res_minus.hard_margins)
    
    fd_dir = (g_plus - g_minus) / (2 * eps)
    ana_dir = J @ d
    
    np.testing.assert_allclose(ana_dir, fd_dir, rtol=1e-4, atol=1e-5)
