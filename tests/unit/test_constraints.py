...
def _make_mock_solution(
    f_hz: float,
    gamma: float | None = None,
    z_in_real: float | None = None,
    z_in_imag: float | None = None,
    v_eom: float | None = None,
    i_source: float | None = None,
) -> CircuitSolution:
    z_in = (
        complex(z_in_real or 0.0, z_in_imag or 0.0)
        if z_in_real is not None or z_in_imag is not None
        else None
    )
...
def test_hard_soft_validation_filtering():
    ...
    extra = [
        ConstraintRecord(
            name="extra_soft", severity="soft", limit=1.0, frequency_scope="all_targets"
        ),
        ConstraintRecord(
            name="extra_valid",
            severity="hard",
...
def test_constraint_evaluations():
    ...
    layout = compile_constraint_layout(
        ...
        extra_records=[],
        target_frequencies_hz=(1e6,),
        evaluation_frequencies_hz=(1e6, 1.5e6),
        target_indices=(0,),
        off_target_indices=(1,),
        severity_filter=ConstraintSeverity.HARD,
        n_cells_b1=1,
    ...
    sols = (
        _make_mock_solution(
            1e6, gamma=0.6, z_in_real=110.0, z_in_imag=60.0, i_source=1.2, v_eom=3.0
        ),
    ...
    g = layout.evaluate(
        ...
        target_indices=(0,),
        off_target_indices=(1,),
        branch1_pole_regions=((0.1, 0.9),),
        branch2_pole_regions=(),
        branch1_k_residues=(1.0,),
        branch2_k_residues=(),
        branch1_f_poles=(5e6,),
        branch2_f_poles=(),
        branch1_l_vals=(2e-6,),
        branch2_l_vals=(),
        branch1_c_vals=(1e-12,),
        branch2_c_vals=(),
        component_limits_l_min=1e-9,
        component_limits_l_max=1e-6,
        component_limits_c_min=1e-15,
        component_limits_c_max=1e-9,
        pole_sep_min_b1=1e6,
        pole_sep_min_b2=1e6,
        z_ref_ohm=50.0,
        gamma_max=0.5,
