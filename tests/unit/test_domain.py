...
def _limits(c_min=1e-12, c_max=1e-6, l_min=1e-9, l_max=1e-3) -> ContinuousLimits:
...
class TestKBoxBounds:
    ...
    def test_infeasible_when_kmin_gt_kmax(self) -> None:
        ...
        _bounds, feasible, reason = _compute_k_box_bounds(1, pole_regions, lims)
        assert not feasible
    ...
    def test_zero_pole_region_uses_c_bound_only(self) -> None:
        ...
        bounds, _feasible, _reason = _compute_k_box_bounds(1, pole_regions, lims)
        lo, _hi = bounds[0]
...
class TestFixedFixedSeparation:
    def test_no_adjacent_fixed_passes(self) -> None:
        ...
        ok, _reason = _check_fixed_fixed_separation(
            f_poles=(5e6, 18e6),
...
class TestDomainHash:
    def _make_topo(self, cells1: int = 1, cells2: int = 1) -> TopologyCandidate:
        ...
        return TopologyCandidate(
            ...
            branch1_cells=cells1,
            branch2_cells=cells2,
