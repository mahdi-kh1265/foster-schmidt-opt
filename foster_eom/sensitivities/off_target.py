...
def compute_v_eom_adjoint_gradient(
    ...
    if eom_id is None or eom_id not in graph.elements:
        raise ValueError(
            f"Off-target V_EOM adjoint gradient requires a valid EOM element. "
            f"graph.eom_element_id={eom_id!r} is missing or not in graph.elements. "
            f"Every production circuit must have eom_element_id set."
        )
